...
Deno.serve(async (req) => {
  ...
  if (!projectUrl || !serviceRoleKey || !supabaseAnonKey) {
    return new Response(
      JSON.stringify({
        error:
  ...
  const headerFields = [
    ...
    ["Date", formatValue(document?.date)],
    ["Issued To", formatValue(document?.issued_to)],
  ...
  for (const item of itemsList) {
    ...
    const unitPriceRaw = item.unit_price ?? item.price ?? null;
  ...
  const path = `variations/${documentId}.pdf`;
