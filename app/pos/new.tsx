...
const createItem = (index: number): PurchaseOrderItem => ({
...
export default function PurchaseOrderScreen() {
  ...
  const [supplier, setSupplier] = useState("Mitre 10");
  const [title, setTitle] = useState("Materials — deck rebuild");
  const [items, setItems] = useState<PurchaseOrderItem[]>([createItem(0)]);
  ...
  const addItem = () => setItems((prev) => [...prev, createItem(prev.length)]);
  ...
  const handleSave = async () => {
    ...
    try {
      const noteParts = [supplier.trim(), title.trim()].filter(Boolean);
      const payload = {
        businessId,
        jobId: job.id,
        clientId: client.id,
      ...
      if (isRls) {
        console.log("po-rls:user", user.id);
        console.log("po-rls:business", businessId);
        console.log("po-rls:payload", {
          clientId: client.id,
          notes: supplier.trim(),
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={[styles.heroStrip, { paddingTop: insets.top + spacing.sm }]}>
          <View style={styles.heroInner}>
            ...
            <Text style={styles.heroSubtitle}>Lock materials early</Text>
          </View>
        </View>

        <Text style={styles.title}>Purchase order</Text>
        <Text style={styles.subtitle}>Simple inputs. Clear totals.</Text>
        ...
        <Card style={styles.card}>
          ...
          <View style={styles.fieldBlock}>
            ...
            <TextInput value={supplier} onChangeText={setSupplier} style={styles.input} />
          </View>

          <View style={styles.fieldBlock}>
            <Text style={styles.fieldLabel}>PO title</Text>
        ...
        {items.map((item, index) => (
          <Card key={item.id} style={styles.itemCard}>
            ...
            <TextInput
              ...
              placeholder="Timber, fixings, hardware"
        ...
        <View style={styles.buttonStack}>
          <PillButton
            label="Attach photo"
            variant="secondary"
            onPress={() => Alert.alert("TODO", "Add photo upload with Supabase Storage.")}
          />
          <PillButton
            label={saving ? "Saving..." : "Save PO"}
            size="lg"
            fullWidth
            onPress={handleSave}
