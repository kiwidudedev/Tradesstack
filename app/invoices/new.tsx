...
const statuses: Array<{ key: "draft" | "sent" | "paid"; label: string }> = [
...
const createItem = (index: number): InvoiceItem => ({
...
export default function InvoiceBuilderScreen() {
  ...
  const [title, setTitle] = useState("Deposit invoice — deck rebuild");
  const [dueDate, setDueDate] = useState("2026-02-10");
  const [items, setItems] = useState<InvoiceItem[]>([createItem(0)]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isRls) {
        console.log("invoice-rls:user", user.id);
        console.log("invoice-rls:business", businessId);
        console.log("invoice-rls:payload", {
          clientId: client.id,
          notes: title.trim(),
          dueDate: dueDate.trim() || null,
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={[styles.heroStrip, { paddingTop: insets.top + spacing.sm }]}>
          <View style={styles.heroInner}>
            ...
            <Text style={styles.heroSubtitle}>Keep cash moving</Text>
          </View>
        </View>

        <Text style={styles.title}>Invoice builder</Text>
        <Text style={styles.subtitle}>Same rhythm as quotes. Faster follow-through.</Text>
        ...
        <Card style={styles.card}>
          ...
          <View style={styles.fieldBlock}>
            <Text style={styles.fieldLabel}>Invoice title</Text>
        ...
        {items.map((item, index) => (
          <Card key={item.id} style={styles.itemCard}>
            ...
            <TextInput
              ...
              placeholder="Deposit, progress claim, variation"
